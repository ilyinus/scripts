...
SELECT 
       GETDATE() [Now],
       COUNT(0) [кол. файлов],
       SUM(user_object_reserved_page_count) * 8.192/1024 AS [UserObjectsMB] ,
       ...
       MIN(unallocated_extent_page_count) * 8.192/1024 AS [min FreeSpaceMB],
       MAX(unallocated_extent_page_count) * 8.192/1024 AS [max FreeSpaceMB]
FROM    sys.dm_db_file_space_usage ;
;
-- 
SELECT
      GETDATE(Now) [time], 
         user_object_reserved_page_count * 8.192/1024 AS [UserObjectsMB] ,
      internal_object_reserved_page_count * 8.192/1024 AS [InternalObjectsMB] ,
      version_store_reserved_page_count * 8.192/1024 AS [VersonStoreMB] ,
      unallocated_extent_page_count * 8.192/1024 AS [FreeSpaceMB],
        *
FROM    sys.dm_db_file_space_usage 
Order by FreeSpaceMB;
/*
Кто может занимать tempdb
